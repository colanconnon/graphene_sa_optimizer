...
book_reader_association_table = db.Table(
    "book_readers",
    db.metadata,
    Column("books_id", db.Integer, db.ForeignKey("books.id")),
    Column("readers_id", db.Integer, db.ForeignKey("readers.id")),
)
...
class Book(SurrogatePK, Model):
    ...
    readers = relationship(
        "Reader", secondary=book_reader_association_table, back_populates="books"
    )
...
class Reader(SurrogatePK, Model):
    ...
    books = relationship(
        "Book", secondary=book_reader_association_table, back_populates="readers"
    )
